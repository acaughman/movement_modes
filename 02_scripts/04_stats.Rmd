...
```{r}
lm1 <- lm(log(home_range) ~ length * diet_troph3 * longevity_wild * temp_preferred, data = filled_data)
lm2 <- lm(pld ~ length * diet_troph3 * longevity_wild * temp_preferred, data = filled_data)
stepAIC(lm1)
stepAIC(lm2)
...
lm1.1 <- lm(
  log(home_range) ~ length + diet_troph3 + longevity_wild + 
    ...
    longevity_wild:temp_preferred + length:diet_troph3:temp_preferred + 
    length:longevity_wild:temp_preferred + diet_troph3:longevity_wild:temp_preferred,
  data = filled_data
)
lm2.1 <- lm(
  pld ~ length + diet_troph3 + longevity_wild + temp_preferred + 
    length:longevity_wild + diet_troph3:longevity_wild + length:temp_preferred + 
...
# look at outliers
outliers_sp <- boxplot.stats(filled_data$sp_richness)$out
boxplot(filled_data$sp_richness, main = "Species Richness by Family", boxwex = 0.1)
mtext(paste("Outliers: ", paste(outliers_sp, collapse = ", ")), cex = 0.4)

# build the model
model_sprich <- aov(sp_richness ~ class2, data = filled_data)
...
# model with log transformed variable
model_sprich_log <- aov(log(sp_richness) ~ class2, data = filled_data)
...
# run Kruskal-Wallis test (non-parametric version of ANOVA)
kruskal.test(sp_richness ~ class2, data = filled_data)
# chi-squared = 28.128, df = 5, p-value = 3.437e-05

# get effect size
filled_data %>% kruskal_effsize(sp_richness ~ class2, ci = TRUE)
# eta2 = 0.0385, SMALL effect

# Kruskal-Wallis is significant, run Dunn's test
dunnTest(sp_richness ~ class2, data = filled_data, method = "bh")
...
# Pearson's Chi-squared test
chisq.test(filled_data$class2, filled_data$iucn_category)
# X-squared = 40.009, df = 25, p-value = 0.0291

# contingency table
iucn_table <- table(filled_data$class2, filled_data$iucn_category)
iucn_table
