...
p1 <- plot_comparisons(lm1, variables = "longevity_wild", condition = c("length", "r_fin"), comparison = "difference") +
  labs(
  ...
  scale_fill_viridis_d(labels = function(x) paste0(round(as.numeric(x), 2)), end = 0.49) +
  scale_color_viridis_d(labels = function(x) paste0(round(as.numeric(x), 2)), end = 0.49) +
  xlim(NA, 150) +
  ylim(-0.25, 0.15)

p2 <- plot_comparisons(lm2, variables = "longevity_wild", condition = c("length", "r_fin")) +
  labs(
    x = "Maximum Lifespan (yr)",
...
p3 <- plot_comparisons(lm1, variables = "longevity_wild", condition = c("length", "temp_preferred"), comparison = "difference") +
  labs(
    x = "Maximum Lifespan (yr)",
    y = "Home Range Difference with Length",
    color = "Optimal Temperature",
    fill = "Optimal Temperature"
  ) +
  theme_bw(base_size = 18) +
  scale_fill_viridis_d(labels = function(x) {
    paste0(round(as.numeric(x), 2))
...
p4 <- plot_comparisons(lm1, variables = "temp_preferred", condition = list("longevity_wild", "r_fin")) +
  labs(
...
p5 <- plot_comparisons(lm1, variables = "temp_preferred", condition = list("length", "r_fin")) +
  labs(
    x = "Optimal Temperature (ºC)",
    y = "Home Range Difference with Length",
    color = "Growth Rate",
...
interact_plot <- (p1 + p2 +p3  ) / ( p4 +p5 + plot_spacer()) + plot_layout(guides = "collect") + plot_annotation(tag_levels = "A")
# interact_plot
