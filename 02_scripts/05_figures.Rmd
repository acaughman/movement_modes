...
life_arrows <- c("decrease", "decrease", "decrease", "equal", "increase",
                 ...
                 "increase", "increase",
                 "increase")
Growth_Rate <- c(0.1035871996367, 0.0891868417293, 0.5697644317228, 0.5148291779984, 0.0000607828670,
...
ggsave(g, file = paste0("stats_sum.pdf"), path = here::here("04_figs", "final_figs"), height = 12, width = 12)
...
missing_data_plot <- fishbase_data %>% 
  dplyr::select(
...
num_panel <- diet_plot + temp_range_plot + oxygen_plot + sp_rich_plot + plot_spacer() + fecund_plot +
...
iucn_plot <- ggplot(iucn, aes(x = iucn_category, y = class2)) + 
  geom_jitter(aes(color = class2)) +
  theme_bw() +
    labs(
      x = "Habitat Type", 
...
cat_panel <- demers_plot + dev_plot + iucn_plot + body_plot +
  plot_layout(ncol = 2, guides = "collect") + 
  plot_annotation(tag_levels = "A") &
  theme(text = element_text(size = 14), legend.position = "none")
