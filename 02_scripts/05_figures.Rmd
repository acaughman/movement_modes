...
genus_filled_data <- read_csv(here::here("01_data", "02_processed_data", "genus_filled_data.csv"))
...
genus_filled <- miss_var_summary(genus_genus_filled_data)
family_filled <- miss_var_summary(genus_filled_data)
...
missing_data_plot <- data %>% 
  ...
  rename(
    ...
    "Maximum Fecundity" = fecundity_max,
...
m1 <- gg_miss_var(missing_data_plot) +
  theme_bw(base_size = 16) +
...
# missing data after genus-level fills
genus_filled_plot <- genus_genus_filled_data %>% 
  dplyr::select(
...
# missing data after femaily-level fills
filled_plot <- genus_filled_data %>%
  dplyr::select(
    oxygen_cons, temp_range, temp_max, fecundity_mean, fecundity_max,
    longevity_wild, temp_preferred, iucn_category
  ) %>%
...
r_fin_plot <- box_data(x = "r_fin", y = "class2") + 
  stat_kruskal_test(label.y = 0.9, label.x = 6, position = "jitter") +
...
sp_rich_plot <- box_data(x = "sp_richness", y = "class2") + 
  stat_kruskal_test(label.y = 1100, position = "jitter") +
  labs(y = "Species Richness", x = "") + 
  scale_x_discrete(position = "top")


num_panel <- r_fin_plot + temp_range_plot + oxygen_plot + sp_rich_plot + plot_spacer() + fecund_plot +
...
numeric_panel
#ggsave(numeric_panel, file = paste0("numeric_panel.pdf"), path = here::here("04_figs", "final_figs"), height = 11, width = 11)
...
expl_cat <- c(
  "body_shape_i", "dev_place", "demers_pelag", "iucn_category","food_mode"
)
...
iucn_plot <- scatter_data(x = "iucn_category", y = "class2") + 
  labs(y = "", x = "IUCN Category") +
  theme(axis.text.y = element_blank())
food_plot <- scatter_data(x = "food_mode", y = "class2") + 
  labs(y = "", x = "Feeding Mode") +
  scale_y_discrete(position = "right")


cat_panel <- demers_plot + dev_plot + iucn_plot + food_plot + plot_spacer() + body_plot +
  plot_layout(ncol = 2, guides = "collect") + 
  plot_annotation(tag_levels = "A") &
  theme(text = element_text(size = 14), legend.position = "none")
...
categorical_panel
#ggsave(categorical_panel, file = paste0("categorical_panel.pdf"), path = here::here("04_figs", "final_figs"), height = 11, width = 11)
