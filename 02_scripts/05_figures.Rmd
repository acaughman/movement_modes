...
missing_filled_data <- filled_data %>% 
  dplyr::select(
    oxygen_cons, temp_range, dev_place, fecundity_mean,
    longevity_wild, temp_preferred, iucn_category,
  ) %>%
  rename(
    "Oxygen Consumption Rate" = oxygen_cons,
...
missing_data_final <- filled_data2 %>%
  dplyr::select(
    oxygen_cons, temp_range, fecundity_mean, dev_place,
    longevity_wild, temp_preferred, iucn_category
  ) %>%
...
m3 <- gg_miss_var(missing_data_final) +
  theme_bw(base_size = 16) +
