...
Lifespan <- c(0.6, 0.0001, 0.6, 0.98, 0.25,
              ...
              0.57, 0.26,
              0.27)
life_arrows <- c("decrease", "decrease", "decrease", "no change", "increase",
...
data1 <- data.frame(cohort, temp_arrows, length_arrows, life_arrows, growth_arrows) %>% 
  ...
  mutate(direction = fct_relevel(direction, c("increase", "no change", "decrease")))
...
gmm_new_fig <- ggplot(fishbase_data_gmm, aes(magnitude_homerange, month_pld)) +
  ...
  labs(
    ...
    color = "Movement Group (Homerange/PLD)",
    tag = "C"
  ) +
  ylim(-0.1,6) +
