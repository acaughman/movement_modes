...
famd_1 <- filled_data %>%
  dplyr::select(
    temp_preferred, temp_max, fecundity_mean, longevity_wild, sp_richness, length,
...
rownames(f1_var) <- c("Optimal Temperature", "Maximum Temperature", "Average Fecundity", "Lifespan", "Species Richness", "Length", 
...
com_data4 <- filled_data %>%
  dplyr::select(
    temp_preferred, temp_max, longevity_wild, sp_richness, length, r_fin, diet_troph3, temp_range, oxygen_cons,
...
rownames(species_scores4) <- c("Optimal Temperature", "Maximum Temperature", "Lifespan", "Species Richness", "Length", 
...
##### NMDS 4 key traits: Maximum temperature, length, longevity, trophic level

```{r}

com_data8 <- filled_data %>%
  dplyr::select(
    temp_max, length, longevity_wild, diet_troph3,
    species, order, class2, class3
  ) %>%
  #mutate(temp_preferred = temp_preferred + 2) %>%
  na.omit()
...
species_scores8 <- as.data.frame(nmds[["species"]])

rownames(species_scores8) <- c("Maximum Temperature", "Length", "Lifespan", "Trophic Level")

species_scores8 <- species_scores8 %>% 
  rownames_to_column(var = "var")
...
nmds8 <- ggplot(filled_scores8, aes(x = NMDS1, y = NMDS2)) +
  geom_vline(xintercept = 0, linetype = "dashed", color = "grey") +
  geom_hline(yintercept = 0, linetype = "dashed", color = "grey") +
  stat_ellipse(geom = "polygon", aes(color = class2, fill = class2), linewidth = 0.25, alpha = 0.2, show.legend = FALSE) +
  geom_point(aes(color = class2), alpha = 0.3) +
  theme_bw(base_size = 22) +
  geom_point(data = centroid8, aes(color = class2), size = 4, alpha = 0.9) +
  labs(x = "nMDS1", y = "nMDS2", color = "Class (home range/PLD)") +
  scale_color_viridis_d() +
  scale_fill_viridis_d() +
  geom_segment(data = species_scores8, aes(x = 0, xend = MDS1, y = 0, yend = MDS2), arrow = arrow(length = unit(0.25, "cm")), linewidth = 0.5, colour = "grey20") +
  ggrepel::geom_label_repel(data = species_scores8, aes(x = MDS1, y = MDS2, label = var), size = 5, fill = "white", cex = 3, direction = "both", segment.size = .25) +
  scale_y_continuous(position = "right") +
  theme(legend.position = "left", panel.grid.major = element_blank(), panel.grid.minor = element_blank()) +
  annotate("text", x = 0.15, y = 0.2, label = "Stress: 0.117")
...
#ggsave(nmds8, file = paste0("nmds_4_traits.pdf"), path = here::here("04_figs", "final_figs"), height = 8, width = 12)
...
adonis2(com_data8$class3 ~ m_com8, permutations = 9999)

pairwise.adonis(m_com8, com_data8$class3, perm = 9999)
