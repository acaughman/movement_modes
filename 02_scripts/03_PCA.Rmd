...
ggsave(f1_fig, file = paste0("famd_all_variables_.pdf"), path = here::here("04_figs", "final_figs"), height = 8, width = 12)
...
nmds_1 <- ggplot(filled_scores, aes(x = NMDS1, y = NMDS2)) +
  ...
  annotate("text", x = 0.17, y = 0.25, label = "Stress: 0.13")

nmds_1

ggsave(nmds_1, file = paste0("nmds_significant_traits_.pdf"), path = here::here("04_figs", "final_figs"), height = 8, width = 12)
...
rownames(species_scores2) <- c("Optimal Temperature", "Length", "Lifespan", "Growth Rate")
...
nmds_2 <- ggplot(filled_scores2, aes(x = NMDS1, y = NMDS2)) +
  ...
  scale_y_continuous(position = "right") +
  theme(legend.position = "left", panel.grid.major = element_blank(), panel.grid.minor = element_blank()) +
  annotate("text", x = 0.2, y = 0.4, label = "Stress: 0.07")
