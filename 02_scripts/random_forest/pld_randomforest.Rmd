...
predict_data <- pld_data_full %>% # predict data is full data set
  select(-PLD) %>%
  distinct(species, .keep_all = TRUE)
```
...
```{r, include=FALSE}
rf_recipe <- recipe(PLD ~ r_fin + kfin + length + diet_troph3 + movement_keyword, data = train) %>% # create prepossessing recipe
...
# write_csv(prediction, here::here("04_results", "pld", "pld_rf_predictions.csv"))
```
