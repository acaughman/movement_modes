...
prediction <- predict(object = rf_fit, new_data = predict_data) %>% # predict the training set
  bind_cols(predict_data) %>% # bind prediction to full data
  rename(predicted_PLD = .pred) %>% # rename prediction column to predicted_PLD
  rename(observed_PLD = PLD) %>% # rename PLD to observed_PLD
  select(species, observed_PLD, predicted_PLD)
