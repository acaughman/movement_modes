...
pld_hr = full_join(hr_data, pld_data) %>% 
  filter(!is.na(predicted_PLD)) %>% 
  filter(!is.na(predicted_homerange))

write_csv(pld_hr, here::here("01_data", "01_raw_data", "homerange_pld.csv"))
```

### Explore data

```{r}
p1 = ggplot(data, aes(log(predicted_homerange), predicted_pld)) +
  geom_point() +
  theme_bw() +
