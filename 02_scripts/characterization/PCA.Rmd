...
f1_var = as.data.frame(f1$var) %>% 
  ...
  mutate(var = row.names(f1_var)) %>% 
  mutate(coord.Dim.1 = case_when(
    var == "temp_preferred" ~ -coord.Dim.1,
    var == "oxygen_cons" ~ -coord.Dim.1,
    var == "r_fin" ~ -coord.Dim.1,
    TRUE ~ coord.Dim.1
  )) %>% 
  mutate(coord.Dim.2 = case_when(
    var == "temp_range" ~ -coord.Dim.2,
    TRUE ~ coord.Dim.2
  ))
...
f1_fig <- fviz_famd_ind(f1, label = "none", habillage = famd_1$class2, palette = hcl.colors(6, "viridis"), addEllipses = TRUE, ellipse.level = 0.95, invisible = "quali.var", alpha.ind = 0.3) +
  theme_bw(base_size = 16) +
  labs(x = "FAMD1 (10.5%)", y = "FAMD2 (9%)", color = "Class (Homerange/PLD)", fill = "Class (Homerange/PLD)", title = NULL) +
  scale_y_continuous(position = "right") +
  theme(legend.position = "left") + 
  geom_segment(data = f1_var, aes(x = 0, xend = coord.Dim.1, y = 0, yend = coord.Dim.2), arrow = arrow(length = unit(0.25, "cm")), colour = "grey10", lwd = 0.3)+
  ggrepel::geom_text_repel(data = f1_var, aes(x = coord.Dim.1, y = coord.Dim.2, label = var), cex = 3, direction = "both", segment.size = 0.25) +
  geom_vline(xintercept = 0, linetype = "dashed", color = "grey") + 
  geom_hline(yintercept = 0, linetype = "dashed", color = "grey")

f1_fig

# ggsave(f1_fig, file = paste0("f1.pdf"), path = here::here("04_figs", "final_figs"), height = 8, width = 18)
```
...
KEEP EITHER 2 or 4, whichever is all significant variables
...
# ggsave(famd_nmds_sig, file = paste0("sig_nmds_famd_panel.pdf"), path = here::here("04_figs", "final_figs"), height = 16, width = 18)
...
com_data7 <- filled_data2 %>%
  dplyr::select(
    ...
    species, order, class2, class
  ) %>%
  mutate(temp_preferred = temp_preferred + 2) %>%
  na.omit()

com7 <- com_data7 %>%
  dplyr::select(-species, -order, -class2, -class)
...
nmds7 <- (nmds_plot7 | nmds_arrows7) +
  plot_annotation(tag_levels = "A")

nmds7

# ggsave(nmds7, file = paste0("nmds7_dispersion.pdf"), path = here::here("04_figs", "final_figs"), height = 8, width = 18)
...
adonis2(com_data7$class ~ m_com7, permutations = 9999)
pairwise.adonis(m_com7, com_data7$class2, perm = 9999)
```
...
com_data5 <- filled_data2 %>%
  dplyr::select(
    longevity_wild, sp_richness, length, kfin, r_fin, oxygen_cons,
    species, order, class2
  ) %>%
  na.omit()
...
com_data6 <- filled_data2 %>%
  dplyr::select(
    longevity_wild, kfin, oxygen_cons,
    species, order, class2
  ) %>%
  na.omit()
