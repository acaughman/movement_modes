...
# this will only work if both variables are numeric
#cor.mat <- round(cor(fishbase_pca2),2) # nothing is highly correlated - good!
#cor.mat
...
famd1 <- FAMD(fishbase_famd1, graph=FALSE)
famd1

eigen_famd1 <- famd1$eig
eigen_famd1 # 6% of variance explained by first 2 components!?
...
famd2 <- FAMD(fishbase_famd2, graph=FALSE)
...
famd3 <- FAMD(fishbase_famd3, graph=FALSE)
famd3

eigen_famd3 <- famd3$eig
eigen_famd3 # 23.34% of variance explained by first 2 components
...
famd4 <- FAMD(fishbase_famd4, graph=FALSE)
famd4

eigen_famd4 <- famd4$eig
eigen_famd4 # 24.66% of variance explained by first 2 components


famd4_fig <- fviz_famd_ind(famd4, label = "none", habillage = fishbase_famd4$class, addEllipses = TRUE, ellipse.level = 0.95) +
  theme_minimal()

famd4_fig # interesting.. groups 1 and 6?
ggsave(famd4_fig, file = paste0("famd4_fig.png"), path = here::here("04_figs"))


```

length seems to be the biggest component so far?


pretty distinct groups here --> but WHOOPS is this supposed to be with PCA?

```{r}

fishbase_famd5 <- read_csv(here::here("01_data", "02_processed_data", "fishbase_data.csv")) %>% 
  select(length, diet_troph3, class) %>% 
  mutate(class = as.factor(class)) %>% 
  na.omit(fishbase_famd5)

famd5 <- FAMD(fishbase_famd5, graph=FALSE)
famd5

eigen_famd5 <- famd5$eig
eigen_famd5 # 34.68% of variance explained by first 2 components


famd5_fig <- fviz_famd_ind(famd5, label = "none", habillage = fishbase_famd5$class, addEllipses = TRUE, ellipse.level = 0.95) +
  theme_minimal()

famd5_fig # interesting.. pretty good groupings here!!
...
famd6 <- FAMD(fishbase_famd6, graph=FALSE)
famd6

eigen_famd6 <- famd6$eig
eigen_famd6 # 22.20% of variance explained by first 2 components
