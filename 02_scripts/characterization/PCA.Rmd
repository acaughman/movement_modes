...
plot2_data = fishbase_data %>% 
  select(demers_pelag, species, class2) %>% 
  mutate(class2 = as.factor(class2))
...
#option 2 sort by class
plot2_data = plot2_data %>% 
  select(oxygen_cons, species, class2)%>% 
  mutate(class2 = sort(class2)) %>% 
  mutate(species = as.factor(species)) %>% 
  mutate(class2 = as.factor(class2)) %>% 
  mutate(species = fct_relevel(plot2_data$species))
...
ggplot(plot2_data, aes(oxygen_cons, class2)) +
  geom_point(aes(color = oxygen_cons))
...
fishbase_p1 <- fishbase_data %>% 
  ...
  select(r_fin, longevity_wild, weight, fecundity_mean, class2) %>% 
  mutate(class2 = as.factor(class2))
...
# ggsave(p1_fig, file = paste0("p1_fig.png"), path = here::here("04_figs"))
...
gridExtra::grid.arrange(p1_c1, p1_c2, ncol = 2)
...
fishbase_p2 <- fishbase_data %>% 
  select(where(is.numeric)) %>% 
  select(r_fin, weight, diet_troph3, class2) %>% 
  mutate(class2 = as.factor(class2))
