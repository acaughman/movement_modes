...
plot1_data = plot1_data %>% 
  select(r_fin, species, class) %>% 
  mutate(class = sort(r_fin)) %>% 
...
plot1_data = plot1_data%>% 
  select(r_fin, species, class) %>% 
  mutate(class = sort(class)) %>% 
  mutate(species = as.factor(species)) %>% 
  mutate(class = as.factor(class)) %>% 
  mutate(species = fct_relevel(plot1_data$species))

#more descriptive
ggplot(plot1_data, aes(r_fin, class)) +
...
plot2_data = fishbase_data %>% 
  select(demers_pelag, species, class) %>% 
  mutate(class = as.factor(class))
...
#more descriptive
ggplot(plot2_data, aes(demers_pelag, class)) +
...
fviz_contrib(f2, choice = "var", axes = 1, top = 10)

# Contributions of variables to PC2
fviz_contrib(f2, choice = "var", axes = 2, top = 10)
