...
famd_1 <- filled_data2 %>%
  dplyr::select(
    ...
    kfin, r_fin, diet_troph3, oxygen_cons, temp_range, body_shape_i, 
...
f1 <- FAMD(famd_1[, 1:15], graph = FALSE)
f1
...
f1_fig <- fviz_famd_ind(f1, label = "none", habillage = famd_1$class2, palette = hcl.colors(6, "viridis"), addEllipses = TRUE, ellipse.level = 0.95, invisible = "quali.var") +
  theme_bw(base_size = 16) +
  labs(x = "FAMD1 (10.5%)", y = "FAMD2 (9%)", color = "Class (Homerange/PLD)", fill = "Class (Homerange/PLD)", title = NULL)
...
#ggsave(plot1, file = paste0("f1.png"), path = here::here("04_figs", "famd_with_arrows"), height = 8, width = 18)
...
famd_4 <- filled_data2 %>%
  dplyr::select(
    temp_preferred, longevity_wild, sp_richness, length, kfin, r_fin, diet_troph3, temp_range, oxygen_cons, demers_pelag, class2
  ) %>%
  mutate(class2 = as.factor(class2)) %>%
  na.omit()
...
famd_6 <- filled_data2 %>%
  dplyr::select(
  temp_preferred, length, longevity_wild, diet_troph3, kfin, demers_pelag, class2
  ) %>%
  mutate(class2 = as.factor(class2)) %>%
  na.omit()
...
com_data4 <- filled_data2 %>%
  dplyr::select(
    temp_preferred, longevity_wild, sp_richness, length, kfin, r_fin, diet_troph3, temp_range, oxygen_cons,
