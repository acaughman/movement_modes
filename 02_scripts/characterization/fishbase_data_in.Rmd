...
library(sf)
library(sp) 
library(raster) #because of this package you have to use dplyr::select specifically
```
...
temp <- stocks() %>% # load stocks info, then
  ...
  dplyr::select(SpecCode, TempMin, TempMax, TempPreferred) %>% # select columns
...
### Collect climate/range data - LOOK INTO FURTHER ------------------------------------------------------------
...
#separates lats and lons
east = eco %>% 
  dplyr::select(spec_code, climate, eastern_lat) %>% 
  rename(lat = eastern_lat) 
west = eco %>% 
  dplyr::select(spec_code, climate, western_lat) %>% 
  rename(lat = western_lat)
north = eco %>% 
  dplyr::select(spec_code, climate, northern_lat) %>% 
  rename(lon = northern_lat)
south = eco %>% 
  dplyr::select(spec_code, climate, southern_lat) %>% 
  rename(lon = southern_lat)

#combines both lons and both lats
lat = rbind(east, west)
lon = rbind(north, south)
eco_sf = full_join(lat, lon, multiple = "all") #join these together so we have 4 rows for species with a point at each corner of it's boundary
...
indian_points = intersect(indian, eco_sf) %>% 
  mutate(ocean = "indian") %>% 
  mutate(basin = "indian")
pacific_points_east = intersect(pacific[4, ], eco_sf) %>% 
  mutate(ocean = "pacific") %>% 
  mutate(basin = "east")
pacific_points_west = intersect(pacific[1:3, ], eco_sf) %>% 
  mutate(ocean = "pacific") %>% 
  mutate(basin = "west")
atlantic_points_east = intersect(atlantic[2,], eco_sf) %>% 
  mutate(ocean = "atlantic") %>% 
  mutate(basin = "east")
atlantic_points_west = intersect(atlantic[1,], eco_sf) %>% 
  mutate(ocean = "atlantic") %>% 
  mutate(basin = "west")
...
fecund <- fecund %>% # combine actual mean with calculated mean
  ...
  dplyr::select(-fecundity_mean_calc)
...
sp_rich <- load_taxa() %>%
  ...
  count(Family) %>%
  rename(family = Family, sp_richness = n)

fish <- left_join(fish, sp_rich)
```

### Collect nocturnal/diurnal data - LOOK INTO FURTHER -----------------------------------------------------

```{r}

# filtering for spec_codes in fish gave only 17 rows

noct <- species() %>%
  dplyr::select(SpecCode, Comments) %>%
  filter(SpecCode %in% fish$spec_code) %>%
  filter(grepl("nocturnal|diurnal|crepuscular", Comments))

# filtering for genus in fish gave 39 rows

noct2 <- species() %>%
  dplyr::select(SpecCode, Genus, Comments) %>%
  filter(Genus %in% fish$genus) %>%
  filter(grepl("nocturnal|diurnal|crepuscular", Comments))


# filtering for "night" using genus in fish gave 116 rows

noct3 <- species() %>%
  dplyr::select(SpecCode, Genus, Comments) %>%
  filter(Genus %in% fish$genus) %>%
  filter(grepl("night", Comments))
```
...
data <- left_join(fish, data) %>%
  dplyr::select(-spec_code, -class, -temp_max, -temp_min)

data <- full_join(data, clusters)

data[data == "pelagic-oceanic"] <- "pelagic_oceanic"
data[data == "pelagic-neritic"] <- "pelagic_neritic"
data[data == "pelagic"] <- "pelagic_oceanic"
```
...
missing_og <- miss_var_summary(data)

missing_data <- data %>%
  dplyr::select(
    oxygen_cons, temp_range, fecundity_max, fecundity_mean,
...
# ggsave(missing_fig, file = paste0("initial_missing_fig.png"), path = here::here("04_figs"))
```
...
filled_data <- filled_data %>%
  ...
  dplyr::select(-food_i.x, -food_i.y)
...
missing_fmax <- fecundity() %>%
  filter(SpecCode %in% fish2$spec_code) %>%
  dplyr::select(SpecCode, FecundityMax) %>%
  janitor::clean_names()

missing_fmax <- left_join(fish2, missing_fmax)

missing_fmax <- missing_fmax %>%
...
missing_fmean <- left_join(fish2, missing_fmean)
...
# 571 missing to now 323 missing
...
corr <- data %>%
  ...
  dplyr::select(
    temp_preferred, temp_range, fecundity_max, fecundity_mean, longevity_wild,
    oxygen_cons, sp_richness, length, kfin, r_fin, diet_troph3
  )
