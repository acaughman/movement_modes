...
```{r}
eco = read_csv(here::here("01_data", "02_processed_data", "ocean_basin.csv"))
...
lat1 = rbind(east, west)
lon1 = rbind(north, south)
lon2 = rbind(south, north)
eco_sf = list(lat1, lon1, lon2) %>% 
  reduce(full_join, multiple = "all")#join these together so we have 4 rows for species with a point at each corner of it's boundary
