...
ocean_data <- read_csv(here::here("01_data", "02_processed_data", "ocean_data.csv"))
```
...
diet <- fooditems(fish$species, fields = c("Species", "FoodI", "FoodII", "FoodIII")) %>%
  ...
  ungroup() %>%
  mutate(food_mode = case_when(grepl(",", FoodI) ~ "generalist",
                               TRUE ~ "specialist")) %>% 
  janitor::clean_names()
...
range <- ocean_data %>% 
  mutate(ocean_basin = case_when(!is.na(basin) ~ paste(basin, ocean),
                                 is.na(basin) ~ ocean)) %>% 
  group_by(species) %>%
  summarise(ocean = paste(sort(unique(ocean)), collapse = ", "),
...
missing_data <- data %>%
  dplyr::select(oxygen_cons, temp_range, fecundity_mean, longevity_wild, 
    temp_preferred, food_i, dev_place, iucn_category, circadian, ocean, basin)
...
filled_data <- filled_data %>%
  ...
  mutate(food_mode = case_when(grepl(",", food_i) ~ "generalist",
                               TRUE ~ "specialist"))
