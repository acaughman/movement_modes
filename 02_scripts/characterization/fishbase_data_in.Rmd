...
### Collect circadian data - LOOK INTO FURTHER? <-----------------------------------------------------
...
circ <- species() %>%
  ...
  mutate(circadian = case_when(SpecCode %in% c(646, 419, 420, 1895, 1917, 14129, 301, 401, 
                                               69, 1494, 1844, 1407, 266, 162, 181, 1080, 
                                               326, 188, 1747, 1342, 513, 6507, 117, 387, 
                                               525, 1363, 5382) ~ "nocturnal",
                               SpecCode %in% c(1893, 5537, 6447, 5604, 18, 216, 227, 785, 
...
# filtering for genus in fish gave 200 obs

circ2 <- species() %>%
  dplyr::select(SpecCode, Genus, Comments) %>%
  filter(Genus %in% fish$genus) %>%
  filter(grepl("nocturnal|diurnal|crepuscular|day|night|daytime|nighttime", Comments))
...
missing_data <- data %>%
  dplyr::select(oxygen_cons, temp_range, fecundity_mean, longevity_wild, 
    temp_preferred, food_i, dev_place, iucn_category, circadian)
...
missing_data_filled <- filled_data %>%
  dplyr::select(
    oxygen_cons, temp_range, fecundity_mean,
...
#ggsave(missing_filled2_fig, file = paste0("missing_filled2_fig.png"), path = here::here("04_figs", "missing_data"))
```
...
corr_filled <- filled_data %>%
  dplyr::select(where(is.numeric)) %>%
  dplyr::select(
    temp_preferred, temp_range, fecundity_mean, longevity_wild,
    oxygen_cons, sp_richness, length, kfin, r_fin, diet_troph3, circadian
  )
