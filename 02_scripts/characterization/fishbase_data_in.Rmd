...
missing <- data %>% 
  select(observed_homerange, oxygen_cons, temp_range, observed_pld, fecundity_max, 
         longevity_wild, temp_preferred, food_i, spawning, spawning_ground, dev_place, rep_guild2)
...
missing_food_i <- fooditems() %>% 
  filter(SpecCode %in% fish2$spec_code) %>% 
  select(SpecCode, FoodI) %>% 
  janitor::clean_names() %>% 
  group_by(spec_code) %>%
  summarise(food_i = paste(unique(food_i), collapse = ", ")) %>% 
  ungroup()

missing_food_i <- left_join(fish2, missing_food_i)

missing_food_i <- missing_food_i %>%
...
filled_data <- filled_data %>% 
        left_join(missing_food_i, by = "genus") %>% 
