...
# 
# eco2 <- eco2 %>% 
#   mutate(northern_lat = as.numeric(northern_lat),
...
fecund <- fecundity() %>% 
  filter(SpecCode %in% fish$spec_code) %>% 
  select(SpecCode, FecundityMax, FecundityMin, FecundityMean) %>% 
  mutate(FecundityMeanCalc = (FecundityMax - FecundityMin)/2) %>% 
  janitor::clean_names()

fecund <- fecund %>%  # average the values for a given spec code
  select(-fecundity_min) %>% 
  group_by(spec_code) %>% 
  mutate(fecundity_max = mean(fecundity_max, na.rm = TRUE), 
...
noct2 <- species() %>% 
  select(SpecCode, Genus, Comments) %>% 
...
data <- left_join(fish, data) %>% 
  select(-spec_code, -class, -temp_max, -temp_min)
...
missing_data <- data %>% 
  select(oxygen_cons, temp_range, fecundity_max, fecundity_mean, 
...
filled_data <- filled_data %>% 
        ...
        select(-fecundity_max.x, -fecundity_max.y)
...
filled_data <- filled_data %>% 
        ...
        select(-fecundity_mean.x, -fecundity_mean.y)
...
corr <- data %>% 
  ...
  select(temp_preferred, temp_range, fecundity_max, fecundity_mean, longevity_wild, 
