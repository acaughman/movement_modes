...
g <- ggplot(data1, aes(x = variable, y = cohort, fill = direction)) +
  ...
  geom_point(data = data1 %>% filter(significant == "yes"), shape = 8, size = 3, color = "white", show.legend = FALSE) +
  ...
  scale_fill_manual(values = c("#86d549","#433e85", "#2c728e"))

g

ggsave(g, file = paste0("stats_sum.png"), path = here::here("04_figs"), height = 12, width = 12)
...
gmm_wsn <- ggplot(fishbase_data_gmm, aes(magnitude_homerange, month_pld)) +
  ...
  geom_jitter(aes(color = class2)) +
  ggtitle("Adult and Larval Movement by Class")

gmm_wsn

#ggsave(gmm_wsn, file = paste0("gmm_wsn.pdf"), path = here::here("04_figs", "WSN"), height = 5, width = 12)
