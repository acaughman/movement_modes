...
gmm_new_fig <- ggplot(fishbase_data_gmm, aes(magnitude_homerange, month_pld)) +
  ...
  geom_label_repel(
    ...
    aes(x = magnitude_homerange, y = month_pld, label = fb_name, color = class2),
  ...
  geom_label_repel(
    data = subset(fishbase_data_gmm, fb_name %in% c("Humphead wrasse", "Pacific red snapper", "Giant seabass")),
    aes(x = magnitude_homerange, y = month_pld, label = fb_name, color = class2),
    # vjust = "outward", hjust = "outward",
    min.segment.length = 0,
    box.padding = 0.7,
  ...
  geom_label_repel(
    data = subset(fishbase_data_gmm, fb_name %in% c("Lemon sole", "Chum salmon", "Pacific ocean perch")),
    aes(x = magnitude_homerange, y = month_pld, label = fb_name, color = class2),
    vjust = "outward", hjust = "outward",
    min.segment.length = 0,
  ...
  geom_label_repel(
    data = subset(fishbase_data_gmm, fb_name %in% c("Atlantic herring", "Atlantic cod", "Haddock")),
    aes(x = magnitude_homerange, y = month_pld, label = fb_name, color = class2),
    vjust = "outward", hjust = "outward",
    min.segment.length = 0,
    nudge_y = 0,
  ...
  geom_label_repel(
    data = subset(fishbase_data_gmm, fb_name %in% c("Atlantic salmon", "California sheephead", "Black seabream")),
    aes(x = magnitude_homerange, y = month_pld, label = fb_name, color = class2),
    vjust = "outward", hjust = "outward",
    min.segment.length = 0,
  ...
  geom_label_repel(
    data = subset(fishbase_data_gmm, fb_name %in% c("Blue marlin", "Yellowfin tuna", "Swordfish")),
    aes(x = magnitude_homerange, y = month_pld, label = fb_name, color = class2),
    vjust = "outward", hjust = "outward",
    min.segment.length = 0,
    nudge_x = 0,
...
m1 <- gg_miss_var(missing_fishbase_data) +
  theme_bw(base_size = 16) +
  labs(y = "Missing Data Before Genus-Level Fill", x = "") +
...
m2 <- gg_miss_var(missing_filled_data) + # figure showing the missing data
  theme_bw(base_size = 16) +
  labs(y = "Missing Data Before Family-Level Fill", x = "Variable") +
  theme(axis.title.x = element_text(vjust = -1))
m2
...
diet_plot <- box_data(x = "diet_troph3", y = "class2") + 
  labs(y = "Trophic Level", x = "Movement Class") +
  scale_x_discrete(position = "top") +
...
individual_panel <- ((temp_plot | length_plot | diet_plot) / (long_plot | kfin_plot)) +
  plot_layout(widths = c(8,8,8,8,8), guides = "collect") +
  plot_annotation(tag_levels = "A") +
  theme_bw(base_size = 16) &
...
ggsave(individual_panel, file = paste0("individual_panel.pdf"), path = here::here("04_figs", "final_figs"), height = 8, width = 13)
