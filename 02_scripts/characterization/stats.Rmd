...
# plot_model(lm1.1, type = "pred")
# plot_model(lm2.1, type = "pred")
#
# median_temp = median(filled_data2$temp_preferred, na.rm=TRUE)
#
# filled_data2 = filled_data2 %>%
#   mutate(temp_bin = case_when(
#     temp_preferred <= median_temp ~ "-1",
#     temp_preferred > median_temp ~ "28"
#   )) %>%
#   mutate(temp_bin = as.factor(temp_bin)) %>%
#   filter(!is.na(temp_bin))

# ggplot(filled_data2, aes(longevity_wild, pld, color = temp_bin)) +
...
ggplot(data = filled_data2, aes(longevity_wild)) +
...
ggsave(interact_plot, file = paste0("interact_plot.png"), path = here::here("04_figs"), height = 8, width = 10)
```
