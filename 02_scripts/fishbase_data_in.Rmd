...
```{r}
data <- read_csv(here::here("01_data", "01_processed_data", "homerange_pld.csv")) %>%
...
```{r}
fish <- load_taxa(server = "fishbase") %>% # load all taxonomic info from fishbase into fish, then
  as.data.frame() %>% 
  filter(Species %in% data$species) %>% # dplyr::select only the rows containing species listed in data
  janitor::clean_names() %>% 
  distinct() %>% 
  select(species, spec_code, genus, family, order) %>% 
  filter(!is.na(family))
```
...
diet <- fooditems(fields = c("Species", "FoodI")) %>% 
  filter(Species %in% data$species) %>% 
  group_by(Species) %>%
  summarise(
...
climate <- ecosystem() %>%
  ...
  group_by(spec_code) %>%
  summarise(
    climate = paste(sort(unique(climate)), collapse = ", ")
  ) 
...
fecund <- fecundity() %>%
  ...
  mutate(FecundityMax = as.numeric(FecundityMax),
         FecundityMin = as.numeric(FecundityMin),
         FecundityMeanCalc = mean(FecundityMax + FecundityMin, na.rm=TRUE)) %>%
...
circ <- species() %>%
  ...
  mutate(circadian = case_when(
    grepl("nocturnal|crepuscular|night|nighttime|evening", Comments) ~ "nocturnal",
    grepl("diurnal|day|daytime", Comments) ~ "diurnal"
  )) %>%
  na.omit() %>%
  janitor::clean_names() %>% 
  group_by(spec_code) %>%
  summarise(
    circadian = paste(sort(unique(circadian)), collapse = ", ")
  ) 
...
data <- cbind(data_bind, fish_bind)
...
missing_fmean <- left_join(fish_genera, missing_fmean, relationship = "many-to-many")
